...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Preview a branch squash merge and optionally delete it.")
    parser.add_argument("branch_name", type=str, help="Branch name to preview and potentially delete.")
    return parser.parse_args()

def run_git_command(command, error_message="Git command failed", expected_return_codes=[0]):
    ...
    try:
        ...
        console.print("[bold red]Error:[/bold red] Git command not found. Is Git installed and in your PATH?")
        sys.exit(1)
...
def preview_branch(branch_name):
    ...
    console.print(f"Attempting to preview branch '[bold cyan]{branch_name}[/bold cyan]' by applying a squash merge...")
    console.print("[yellow]This will modify your working directory. Check the changes with 'git status' or 'git diff'.[/yellow]")
    
    # Allow return codes 0 (success) and 1 (conflicts)
    result = run_git_command(
        ["git", "merge", "--squash", "--no-commit", "--strategy-option", "theirs", branch_name],
        error_message=f"Failed to preview branch '{branch_name}'. Does it exist?",
        expected_return_codes=[0, 1] 
    )

    if result.returncode == 0:
        console.print("[green]Preview successful. Working directory updated cleanly.[/green]")
    elif result.returncode == 1 and "Automatic merge failed" in result.stdout:
         console.print("[yellow]Preview generated merge conflicts.[/yellow] Please inspect the conflicts in your working directory.")
...
def delete_branch(branch_name):
    ...
    console.print(f"[green]Branch '[bold cyan]{branch_name}[/bold cyan]' deleted successfully.[/green]")
...
def main():
    ...
    try:
        ...
        confirm = input(f"Delete branch '{branch_name}' after reviewing the preview? (y/n): ").lower()
        if confirm == "y":
